...
def sell():
    ...
    rows = db.execute("SELECT DISTINCT(symbol) as symbol FROM transactions WHERE user_id = ? AND (SUM(CASE WHEN type='buy' THEN amount ELSE 0 END) - SUM(CASE WHEN type='sell' THEN amount ELSE 0 END)>0)", session.get("user_id"))
    if len(rows) < 1:
    ...
    if request.method == "POST":
        ...
        new_saldo = int(user_cash[0]['cash'])+int(amount_possesion[0]['amount']) * quote['price']
